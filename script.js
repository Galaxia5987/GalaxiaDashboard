...
setInterval(() => {
    ...
    if (bruh == undefined) {
        x = map(3, 0, 16.46, 78, 775);
        y = map(3, 0, 8.23, 480, 137);
        document.getElementById("swerve").style.left = x + "px";
        document.getElementById("swerve").style.top = y + "px";
        document.getElementById("swerve").style.transform = "rotate(" + 90 + "deg)"
        return;
    }
    x = Number.parseFloat(bruh.slice(0, bruh.indexOf(',')));
    y = Number.parseFloat(bruh.slice(bruh.indexOf(',') + 2, bruh.length));
    x = map(x, 0, 16.46, 125, 750);
    y = map(y, 0, 8.23, 461, 158);
    document.getElementById("swerve").style.left = x + "px";
    document.getElementById("swerve").style.top = y + "px";
    drivetrain_rotation = NetworkTables.getValue("/SmartDashboard/drivetrain-rotation");
    if (drivetrain_rotation == undefined) return;
    rotation = -Number.parseFloat(drivetrain_rotation.slice(0, drivetrain_rotation.indexOf(" "))) + 90;
...
function mapX(x) {
    return map(x, 0, 16.46, 78, 775);
}

function mapY(y) {
    return map(y, 0, 8.23, 480, 137);
}
